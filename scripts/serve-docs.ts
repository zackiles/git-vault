...
queueMicrotask(async () => {
  ...
  if (!cmd) {
    throw new Error(`Failed to open docs at '${url}. Unsupported platform: ${Deno.build.os}`)
  }
  const [bin, ...args] = [...cmd.split(' '), url]
  const { success, code } = await new Deno.Command(bin, { args }).output()
  if (!success) throw new Error(`Failed to open docs at '${url}' (exit ${code})`)
})
...
Deno.serve(
  { port },
  (req) => serveDir(req, { fsRoot: './docs/luna', showIndex: true, quiet: true }),
)
console.clear()
